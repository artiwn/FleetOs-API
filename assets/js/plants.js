...
function plants(){
  if (Array.isArray(window.FleetOSLivePlants) && window.FleetOSLivePlants.length) return window.FleetOSLivePlants;
  const stored = JSON.parse(localStorage.getItem('fleetos_demo_plants') || 'null');
  if (stored && stored.length) return stored;
  localStorage.setItem('fleetos_demo_plants', JSON.stringify(demoPlantSeed));
  return demoPlantSeed;
}
function savePlants(list){ localStorage.setItem('fleetos_demo_plants', JSON.stringify(list)); }
function plantStatusCls(v){
  v = String(v).toLowerCase();
  if (v.includes('fault') || v.includes('offline')) return 'danger';
  if (v.includes('warning') || v.includes('delayed')) return 'warning';
  return 'success';
}
function selectedPlant(){
  const id = localStorage.getItem('fleetos_selected_plant');
  return plants().find(p => p.id === id) || plants()[0];
}
function plantRows(list){
  return `<div class="data-table plant-table"><div class="data-head"><span>Plant</span><span>Tenant / Source</span><span>Location</span><span>Capacity</span><span>Status</span><span>Actions</span></div>${list.map(p => `<div class="data-row" data-id="${p.id}"><div><strong>${p.name}</strong><small>${p.code}<br>${p.id}</small></div><div><strong>${p.tenant}</strong><small>${p.vendor} · ${p.integration}</small></div><div><strong>${p.country}</strong><small>${p.region} · ${p.city}</small></div><div><strong>${p.capacityDc} MWp DC</strong><small>${p.capacityAc} MW AC · ${p.devices} devices</small></div><div><span class="badge ${plantStatusCls(p.status)}">${p.status}</span><small>${p.alerts} alerts · ${p.lastData}</small></div><div class="row-actions"><button data-action="open">Open</button><button data-action="devices">Devices</button><button data-action="telemetry">Telemetry</button><button data-action="alerts">Alerts</button></div></div>`).join('')}</div>`;
}
const assetClientOptions = [
  { name:'Arpi Solar Group', tenant:'Tenant Alpha Energy', country:'Armenia', region:'Yerevan', city:'Yerevan', timezone:'Asia/Yerevan', contact:'Anna Grigoryan' },
  { name:'North Valley Client', tenant:'Tenant North Operations', country:'Armenia', region:'Armavir', city:'Armavir', timezone:'Asia/Yerevan', contact:'Karen Petrosyan' },
  { name:'Gamma Grid Holdings', tenant:'Tenant Gamma Grid', country:'Spain', region:'Madrid', city:'Madrid', timezone:'Europe/Madrid', contact:'Miguel Santos' },
  { name:'Lyon Energy Client', tenant:'Tenant Delta Enterprise', country:'France', region:'Auvergne-Rhône-Alpes', city:'Lyon', timezone:'Europe/Paris', contact:'Claire Martin' }
];
...
const vendorFlowConfig = {
  ...
  'Other / Manual': {
    hint: 'Manual source stores a FleetOS draft record and waits for adapter mapping later.',
    steps: [
...
function vendorFieldControl(field){
  ...
  if (field.type === 'select') return `<label${cond}>${label}<select name="${name}"${id}${req}>${(field.options||[]).map(x=>`<option>${x}</option>`).join('')}</select></label>`;
...
function assetStepContent(){
  return `<section class="form-section-card asset-create-step-panel" data-asset-panel="plant-profile">
    <div class="section-title"><div><h3>Vendor Platform</h3><p class="muted">This wizard creates a Plant. Select the vendor/source platform, then FleetOS loads that vendor's own plant creation logic.</p></div><span class="badge success">Required</span></div>
    <input type="hidden" name="assetType" id="assetTypeSelect" value="Plant" />
    <div class="client-form-grid two-col">
      <label>Source Scheme<select name="sourceScheme" id="assetSourceScheme">${sourceSchemes.map(x=>`<option>${x}</option>`).join('')}</select></label>
      <label>Record Status<select name="status"><option>Draft</option><option>Pending Review</option><option>Normal</option><option>Inactive</option></select></label>
      <label>Creation Mode<select name="creationMode"><option>Manual vendor-driven form</option><option>From Discovery</option><option>Import from vendor source</option></select></label>
      <label>Payload Strategy<input readonly value="Core FleetOS fields + selected vendor fields only"></label>
    </div>
...
function clientStepContent(){
  return `<section class="form-section-card asset-create-step-panel" data-asset-panel="client">
    <div class="section-title"><div><h3>Client Assignment</h3><p class="muted">Select the client first. FleetOS auto-fills the managing tenant and default location context from Client Registry.</p></div></div>
    <div class="client-form-grid two-col">
      <label>Client<select name="client" id="assetClientSelect">${assetClientOptions.map(c=>`<option>${c.name}</option>`).join('')}</select></label>
      <label>Managing Tenant<input name="tenant" id="assetTenantAuto" readonly value="${assetClientOptions[0].tenant}"></label>
      <label>Client Contact<input name="clientContact" id="assetContactAuto" readonly value="${assetClientOptions[0].contact}"></label>
      <label>Default Time Zone<input name="timezone" id="assetTimezoneAuto" readonly value="${assetClientOptions[0].timezone}"></label>
      <label>Default Country<input name="country" id="assetCountryAuto" readonly value="${assetClientOptions[0].country}"></label>
      <label>Default Region<input name="region" id="assetRegionAuto" readonly value="${assetClientOptions[0].region}"></label>
    </div>
  </section>`;
}
...
function reviewStepContent(){
  return `<section class="form-section-card asset-create-step-panel" data-asset-panel="review">
    <div class="section-title"><div><h3>Review</h3><p class="muted">Create the FleetOS plant record and selected vendor payload draft. Hidden vendor fields are disabled and excluded.</p></div><span class="badge warning">Draft review</span></div>
    <div class="info-grid asset-create-review"><div><span>Group</span><strong id="reviewAssetType">Plant</strong><small>Created inside Groups → Plants</small></div><div><span>Client</span><strong id="reviewClient">${assetClientOptions[0].name}</strong><small>Selected owner/customer</small></div><div><span>Managing Tenant</span><strong id="reviewTenant">${assetClientOptions[0].tenant}</strong><small>Auto-filled from client</small></div><div><span>Source Scheme</span><strong id="reviewSourceScheme">Huawei FusionSolar</strong><small>Selected vendor creation flow</small></div><div><span>Visible Vendor Flow</span><strong id="reviewVendorAllowed">Basic Info · Add Devices · String Capacity · Other Info</strong><small>Only these sections contribute to vendor payload</small></div><div><span>Payload Rule</span><strong>Common FleetOS fields + selected vendor fields only</strong><small>No Solis fields are sent to Huawei, no Sungrow fields are sent to Deye, etc.</small></div></div>
  </section>`;
}
function plantCreateModal(){
  return `<div class="modal plant-create-modal" id="plantCreateModal" aria-hidden="true">
    <div class="modal-card wide-modal client-create-card asset-create-card">
      <button class="modal-close" id="closePlantCreate" type="button">x</button>
      <div class="client-create-head">
        <div><p class="eyebrow">Groups · Plants</p><h2>Create Plant</h2><p class="muted">Select client and vendor platform. FleetOS then loads the vendor-specific plant creation flow and conditions.</p></div>
        <span class="badge warning">Draft</span>
      </div>
      <form id="plantCreateForm" class="plant-create-form client-create-form setup-layout asset-create-form">
...
function getSidebarGroups(){
  ...
  return defaultSidebarGroups.map(g => ({...g, objects:g.key === 'plants' ? plants().length : g.objects}));
}
function saveSidebarGroups(groups){
  localStorage.setItem('fleetos_sidebar_groups', JSON.stringify(groups));
}
...
function renderPlants(){
  ...
  return `<section class="page-hero"><div><p class="eyebrow">Global Admin · Groups</p><h1>${openSolar ? 'Plant Registry' : 'Groups'}</h1><p class="muted">Groups control which typed registries appear in the main sidebar. Each group keeps its own table, filters, detail page and create form.</p></div>${openSolar ? `<button class="create-action" id="openPlantCreate" type="button"><span class="pulse"></span><div><strong>+ Add Plant</strong><small>Client → vendor → plant data</small></div></button>` : `<button class="create-action" id="openGroupCreate" type="button"><span class="pulse"></span><div><strong>+ Create Group</strong><small>Show/hide in sidebar</small></div></button>`}</section>
  <section class="context-bar glass-card"><button class="ctx-item"><span>Visible Groups</span><strong>${groups.filter(g => g.show).length}</strong></button><button class="ctx-item"><span>Total Groups</span><strong>${groups.length}</strong></button><button class="ctx-item"><span>Plants</span><strong>${list.length}</strong></button><button class="ctx-item"><span>Attention</span><strong>${warnings}</strong></button><button class="ctx-item"><span>Installed Capacity DC</span><strong>${totalMw} MWp</strong></button></section>
  ...
  <section class="panel glass-card ${openSolar ? '' : 'hidden'}" id="solarPlantsRegistryView"><div class="panel-head"><div><p class="eyebrow">Groups · Plants</p><h2>Plant Registry</h2><p>Plants have their own columns, filters, detail page and Create Plant wizard.</p></div><div class="hero-actions"><button class="secondary-btn" id="backToGroups" type="button">Back to Groups</button><button class="create-action" id="openPlantCreateInline" type="button"><span class="pulse"></span><div><strong>+ Add Plant</strong><small>Client → vendor → plant data</small></div></button></div></div><div class="toolbar plant-registry-toolbar"><input id="plantSearch" placeholder="Search plants, tenants, vendors..."/><select id="plantStatusFilter"><option>All Statuses</option><option>Normal</option><option>Warning</option><option>Fault</option><option>Pending Review</option><option>Draft</option></select><select id="plantVendorFilter"><option>All Vendors</option><option>Huawei FusionSolar</option><option>Deye / Solarman</option><option>GoodWe SEMS</option><option>SolisCloud</option><option>SolaX Cloud</option><option>Peimar</option><option>Sungrow iSolarCloud</option><option>Sofar</option><option>Other / Manual</option><option>Huawei</option><option>Sungrow</option><option>Solis</option><option>Deye</option></select></div><div id="plantTable">${plantRows(list)}</div></section>
...
function wirePlants(){
  ...
  function apply(){
    ...
    table.innerHTML = plantRows(list);
  }
  search?.addEventListener('input', apply); status?.addEventListener('change', apply); vendor?.addEventListener('change', apply);
  table?.addEventListener('click', e => {
    const btn = e.target.closest('button');
    const row = e.target.closest('.data-row'); const id = row?.dataset.id;
    const p = plants().find(x => x.id === id);
    if (id) localStorage.setItem('fleetos_selected_plant', id);
    if (!btn && id) { location.href = 'plant-detail.html'; return; }
    if (!btn) return;
    if (btn.dataset.action === 'open') location.href = 'plant-detail.html';
    if (btn.dataset.action === 'devices') { localStorage.setItem('fleetos_device_filter_plant', id); FleetLayout.toast('Opening Device Registry for selected plant'); location.href = 'devices.html'; }
    if (btn.dataset.action === 'telemetry' && p) { localStorage.setItem('fleetos_telemetry_context', JSON.stringify({tenant:p.tenant, plant:p.name, device:'All Devices', metric:'Current Power', range:localStorage.getItem('fleetos_time')||'Last 24h', layer:'Normalized'})); location.href = 'telemetry.html'; }
    if (btn.dataset.action === 'alerts' && p) { localStorage.setItem('fleetos_alert_context', JSON.stringify({plantId:p.id, tenant:p.tenant})); location.href = 'alerts.html'; }
  });
  ...
  document.getElementById('groupCreateModal')?.addEventListener('click', e => { if (e.target.id === 'groupCreateModal') closeGroupCreateModal(); });
  document.getElementById('groupsTableHost')?.addEventListener('change', e => {
    const input = e.target.closest('[data-group-toggle]');
    if (!input) return;
  ...
  document.getElementById('plantCreateModal')?.addEventListener('click', e => { if (e.target.id === 'plantCreateModal') closePlantCreateModal(); });

  const allStepButtons = Array.from(document.querySelectorAll('[data-asset-step]'));
  const baseStepKeys = ['client','plant-profile'];
  let activeStep = 'plant-profile';

  function isSolarSelected(){ return (document.getElementById('assetTypeSelect')?.value || 'Plant') === 'Plant'; }
  function selectedSourceScheme(){
    return document.getElementById('assetSourceScheme')?.value || sourceSchemes[0];
  }
  function selectedFlow(){
    return vendorFlowConfig[selectedSourceScheme()] || vendorFlowConfig['Other / Manual'];
  }
  ...
  function visibleSteps(){
    return allStepButtons.filter(b => !b.hidden).map(b => b.dataset.assetStep);
  }
  ...
  function renderVendorFlow(){
    ...
    allStepButtons.forEach((btn, index) => {
      ...
      const b = btn.querySelector('b'); if (b) b.textContent = order;
  ...
  function showAssetStep(step){
    ...
    activeStep = step;
    allStepButtons.forEach(b => b.classList.toggle('active', b.dataset.assetStep === step));
    ...
    document.getElementById('prevAssetStep').disabled = ix <= 0;
    document.getElementById('nextAssetStep').classList.toggle('hidden', activeStep === 'review');
    document.getElementById('submitAssetCreate').classList.toggle('hidden', activeStep !== 'review');
    updateAssetReview();
  }
  function selectedClient(){
    const name = document.getElementById('assetClientSelect')?.value || assetClientOptions[0].name;
    return assetClientOptions.find(c => c.name === name) || assetClientOptions[0];
  }
  ...
  allStepButtons.forEach(b => b.addEventListener('click', () => showAssetStep(b.dataset.assetStep)));
  ...
  document.getElementById('assetTypeSelect')?.addEventListener('change', () => {
    if (!isSolarSelected()) document.getElementById('assetSourceScheme').value = 'Other / Manual';
  ...
  document.getElementById('plantCreateModal')?.addEventListener('change', e => {
    if (['huaweiPlantType','huaweiEvOnly','solisTariffType','solisCountry','solisPlantType','solisCountry','solisPlantType','goodwePlantType','sungrowPlantType','sungrowConsumptionMode','sungrowFeedMode','solaxPlantType','solaxDeviceImport','deyeImageMode','deyeDeviceMethod','deyeImageMode','deyeDeviceMethod','sofarSystemType','peimarDeviceMethod'].includes(e.target.id)) {
      applyVendorConditionals();
    }
  });
  syncClientDefaults();
  ...
  document.getElementById('plantCreateForm')?.addEventListener('submit', e => {
    ...
    const newPlant = {
      ...
      externalId: 'MANUAL',
      assetType: type,
      code: fd.get('code') || `AST-${String(now).slice(-4)}`,
      name: (isSolar ? fd.get('name') : fd.get('genericName')) || `New ${type}`,
      ...
      vendor: fd.get('sourceScheme') || 'Other / Manual',
      vendorPayloadRule: 'core + selected vendor fields only',
      status: fd.get('status') || 'Draft',
      type: isSolar ? (fd.get('type') || 'Commercial') : type,
      country: fd.get('countryManual') || client.country,
      region: client.region,
      city: fd.get('city') || client.city,
      address: fd.get('address') || '—',
      lat:'—', lng:'—', timezone: fd.get('timezoneManual') || client.timezone,
      ...
      livePower:'0 kW', today:'0 kWh', month:'0 kWh', pr:'—', lastData:'Manual record', freshness:'Pending', commissioned: fd.get('commissioned') || '—',
      owner: client.name, operator: client.tenant, om: fd.get('serviceProvider') || client.tenant
    };
    const list = plants();
    list.unshift(newPlant);
    savePlants(list);
    localStorage.setItem('fleetos_selected_plant', newPlant.id);
    FleetLayout.toast(`Plant created as draft`);
...
function wirePlantDetail(){
  ...
  document.querySelectorAll('[data-plant-tab]').forEach(b => b.onclick = () => {
    ...
    document.getElementById('plantDetailContent').innerHTML = plantTab(p, tab);
  });
}

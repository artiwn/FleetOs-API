const V51 = (() => {
  ...
  const tone = v => /Critical|P1|Escalated|Failed|Warning/.test(v) ? 'danger' : /High|P2|Pending|Delayed|In Review|Assigned|Scheduled/.test(v) ? 'warning' : 'success';
  function hero(eyebrow, title, desc, status='Updated now') { return `<section class="page-hero"><div><p class="eyebrow">${eyebrow}</p><h1>${title}</h1><p class="muted">${desc}</p></div><button class="freshness-card" onclick="FleetLayout.toast('${title} refreshed')"><span class="pulse"></span><div><strong>${status}</strong><small>v5.1 enterprise workspace</small></div></button></section>`; }
  function kpis(items){ return `<section class="module-grid">${items.map(i=>`<article class="module-card"><span>${i.label}</span><strong>${i.value}</strong><small>${i.note}</small></article>`).join('')}</section>`; }
  function drawer(title, item, extra='') { let d=document.getElementById('detailDrawer'); if(!d){ d=document.createElement('aside'); d.id='detailDrawer'; d.className='detail-drawer'; document.body.appendChild(d);} d.innerHTML=`<button class="drawer-close" type="button">x</button><p class="eyebrow">${title}</p><h2>${item.title||item.action}</h2><div class="drawer-body"><div class="drawer-status-row"><span class="badge ${tone(item.severity||item.priority||item.result||item.status)}">${item.severity||item.priority||item.result||item.status}</span><span class="badge warning">${item.status||item.entity||''}</span></div><p>${item.desc||'Enterprise workflow object with traceability, ownership, SLA and audit context.'}</p><div class="drawer-metrics rich">${Object.entries(item).filter(([k])=>!['desc','title'].includes(k)).slice(0,8).map(([k,v])=>`<div><span>${k}</span><strong>${v}</strong></div>`).join('')}</div>${extra}<div class="timeline-mini"><strong>Audit Timeline</strong><p>Created by system normalization</p><p>Assigned owner and SLA policy</p><p>Latest user action captured in audit trail</p></div></div><div class="drawer-actions"><button class="primary-action" onclick="FleetLayout.toast('Action saved')">Save Action</button><button class="secondary-action drawer-close-2">Close</button></div>`; d.classList.add('open'); d.querySelectorAll('.drawer-close,.drawer-close-2').forEach(b=>b.onclick=()=>d.classList.remove('open')); }
  return { cases, tickets, orders, audit, tone, hero, kpis, drawer };
})();

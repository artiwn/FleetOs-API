...
const taskStoreKey = 'fleetos_tasks_v2';
const taskStatuses = ['New', 'Assigned', 'In Progress', 'Scheduled', 'Completed', 'Closed'];
const taskPriorities = ['P1', 'P2', 'P3', 'P4'];
const taskTeams = ['Operations', 'Field Service', 'Data Operations', 'Maintenance', 'Support'];
const taskAssignees = ['Unassigned', 'Operations Team', 'Field Team A', 'Field Team B', 'BESS Specialist', 'Data Operations', 'Support Manager'];


const taskContextCatalog = [
  {
...
function contextClientNames() { return taskContextCatalog.map(c => c.client); }
function contextForClient(client) { return taskContextCatalog.find(c => c.client === client) || taskContextCatalog[0]; }
function contextPlant(client, plantId) {
  const ctx = contextForClient(client);
  return ctx.plants.find(p => p.id === plantId) || ctx.plants[0];
}
function contextDevice(client, plantId, deviceId) {
...
function nowLabel() { return new Date().toLocaleString([], { month: 'short', day: '2-digit', hour: '2-digit', minute: '2-digit' }); }
...
function normalizeChecklist(items) {
  return (items || []).map((item, index) => typeof item === 'string' ? { text: item, done: index === 0 } : item);
}
...
function saveStoredTasks(items) { localStorage.setItem(taskStoreKey, JSON.stringify(items.map(normalizeTask))); }
function allTasks() { return storedTasks(); }
...
function selectedTask() {
  ...
  const id = params.get('id') || localStorage.getItem('fleetos_selected_task') || allTasks()[0]?.id;
  return allTasks().find(t => t.id === id) || allTasks()[0];
}
function openTask(id) {
  localStorage.setItem('fleetos_selected_task', id);
  location.href = `task-detail.html?id=${encodeURIComponent(id)}`;
}
function newTaskId(type = 'TASK') {
...
function taskKpis(items = filteredTasks()) {
...
function renderTaskRows(items = filteredTasks()) {
...
function renderCreateTaskModal() {
  const firstClient = taskContextCatalog[0];
  const firstPlant = firstClient.plants[0];
  const firstDevice = firstPlant.devices[0];
  const firstAlert = firstPlant.alerts[0];
  return `<div class="modal" id="taskModal"><div class="modal-card compact-modal"><button class="modal-close" type="button" onclick="closeTaskModal()">x</button>
    <h2>Create Task / Work Order</h2><p class="muted">Select the operational context first. FleetOS will auto-fill plant, device, alert and suggested work details.</p>
    <form id="taskCreateForm" class="workflow-form">
      <div class="form-grid">
        ...
        <label class="full">Notes<textarea name="description" id="taskDescriptionInput" rows="4" placeholder="Optional note for the assigned team. FleetOS already links the selected context."></textarea></label>
      </div>
...
function wireTasksPage() {
  ...
  document.getElementById('taskCreateForm')?.addEventListener('submit', e => {
    ...
    saveTask(task); closeTaskModal(); refreshTasksPage(); FleetLayout.toast(`${task.id} created`);
  });
}
function populateTaskContextSelects() {
  ...
  const client = clientEl.value || taskContextCatalog[0].client;
...
function updateTaskContextPreview() {
  const client = document.getElementById('taskClientSelect')?.value || taskContextCatalog[0].client;
  ...
  if (preview) preview.innerHTML = `<strong>Auto-filled context</strong><p><b>${client}</b> → ${plant.name} → ${device.name}${alert ? ` → ${alert.id} (${alert.severity})` : ' → manual task'}</p><small>FleetOS will save client, plant ID, device ID and related alert automatically.</small>`;
}
function wireTaskCreateContext() {
  ...
  document.getElementById('taskTitleInput')?.addEventListener('input', e => { e.currentTarget.dataset.edited = 'true'; e.currentTarget.dataset.autofill = 'false'; });
}
...
function assignTaskModalMarkup(t) {
  return `<div class="modal open" id="assignTaskModal"><div class="modal-card compact-modal"><button class="modal-close" type="button" onclick="closeAssignTaskModal()">x</button>
    ...
    <form id="assignTaskForm" class="workflow-form">
      <div class="form-grid">
...
function wireTaskDetail() {
  ...
  document.querySelectorAll('.detail-tabs button').forEach(btn => btn.onclick = () => {
    ...
    t = selectedTask();
    document.getElementById('taskDetailContent').innerHTML = taskDetailTab(t, btn.dataset.tab);
    bindTaskDetailActions(t);
  });
  bindTaskDetailActions(t);
}
function bindTaskDetailActions(t) {
  document.getElementById('openTaskPlant')?.addEventListener('click', () => { localStorage.setItem('fleetos_selected_plant', t.plantId); location.href = 'plant-detail.html'; });
  document.getElementById('openTaskDevice')?.addEventListener('click', () => { localStorage.setItem('fleetos_selected_device', t.deviceId); location.href = 'device-detail.html'; });
  document.getElementById('openTaskAlert')?.addEventListener('click', () => { localStorage.setItem('fleetos_selected_alert', t.alertId); location.href = `alert-detail.html?id=${encodeURIComponent(t.alertId)}`; });
}
function rerenderTaskDetail(id) {
...
function addTaskEvidence(id) {
  const input = document.getElementById('taskEvidenceInput');
  const value = input?.value.trim(); if (!value) return;
  const t = allTasks().find(x => x.id === id); if (!t) return;
  saveTask({ ...t, evidence: [...(t.evidence || []), value], notes: [...(t.notes || []), 'Evidence added.'] });
  rerenderTaskDetail(id);
}
function toggleChecklist(id, index) {
  const t = allTasks().find(x => x.id === id); if (!t) return;

const FleetRBAC = (() => {
  ...
  const roles = [
    { name:'Platform Administrator', level:'Global', users:3, scope:'FleetOS platform governance', permissions:'Tenants, integrations, audit, RBAC, data governance, system configuration', sensitive:'Yes', status:'Active', approval:'Two-step approval', modules:['Tenant Registry','Integrations','Data Governance','Audit Center','Users & Access','Platform Settings'] },
  ...
  const policies = [
    { title:'Global scope', type:'Scope boundary', status:'Active', owner:'Security Office', rule:'Only FleetOS platform administrators can hold this scope.', text:'Cross-tenant governance, audit, integration and platform configuration. This scope is never inherited from tenant roles.' },
  ...
  const roleModules = ['Tenant Management','Client Management','Plant Registry','Device Registry','Integrations','Billing & Payments','Reports','Audit Center','RBAC','Data Governance','Command Center','Platform Operations'];
  const roleActions = ['View','Create','Edit','Delete','Export','Approve','Execute','Configure'];
  const versionDiff = [
  ...
  let activeTab = localStorage.getItem('fleetos_rbac_tab') || 'overview';

  function esc(value) {
    return String(value ?? '').replace(/[&<>"']/g, ch => ({ '&':'&amp;', '<':'&lt;', '>':'&gt;', '"':'&quot;', "'":'&#39;' }[ch]));
  }
  function tone(v) {
    if (/Active|Enabled|Passed|Low|Approved|Accepted|Standard|Never/.test(v)) return 'success';
    if (/Invited|Required|Due|Medium|Pending|Open|Sensitive|Attention|Review/.test(v)) return 'warning';
    if (/Suspended|Disabled|Blocked|High|Expired|Archived/.test(v)) return 'danger';
    return 'info';
  }
  function pill(v) { return `<span class="badge ${tone(v)}">${esc(v)}</span>`; }
  function cell(title, sub='') { return `<div><strong>${esc(title)}</strong>${sub ? `<small>${esc(sub)}</small>` : ''}</div>`; }
  function notify(message) { if (window.FleetLayout?.toast) FleetLayout.toast(message); }
  function openUser(id) { localStorage.setItem('fleetos_selected_access_user', id); window.location.href = 'user-access-detail.html'; }
  ...
  function matrixTab() {
    return `
      <div class="section-title-v17 rbac-section-head"><div><h2>Permission Matrix</h2><p class="muted">Governance matrix for core FleetOS modules and role layers.</p></div><button class="small-btn ghost" data-toast="Permission matrix exported">Export Matrix</button></div>
  ...
  function filteredUsers() {
    ...
    const status = document.getElementById('rbacStatus')?.value || '';
    const scope = document.getElementById('rbacScope')?.value || '';
  ...
  function userTable(list, compact=false) {
  ...
  function drawUsers() { const wrap = document.getElementById('rbacUserTableWrap'); if (wrap) wrap.innerHTML = userTable(filteredUsers()); }

  function drawerShell(title, body, actions='') {
  ...
  function createRoleWizard() {
    drawerShell('Create Role', `
      ...
      <div class="rbac-step-list rbac-step-list-wide">
        ...
        <article><b>3</b><div><strong>Modules</strong><small>FleetOS workspaces</small></div></article>
      ...
      <div class="form-grid wizard-step active rbac-form-grid">
        ...
        <label>Role Type<select><option>Internal FleetOS role</option><option>Tenant role</option><option>Client role</option><option>External service role</option></select></label>
  ...
  function roleDrawer(index=null) {
  ...
  function compareVersionsDrawer() {
  ...
  function inviteReviewDrawer(index) {
  ...
  function bindRoot() {
    ...
    root.addEventListener('click', e => {
      const tab = e.target.closest('[data-rbac-tab]');
      if (tab) {
        const nextTab = tab.dataset.rbacTab;
        if (nextTab !== activeTab) {
          activeTab = nextTab;
          localStorage.setItem('fleetos_rbac_tab', activeTab);
          updateActiveTabs();
          drawTab();
        }
        return;
      }
      const jump = e.target.closest('[data-rbac-tab-jump]');
      if (jump) { activeTab = jump.dataset.rbacTabJump; localStorage.setItem('fleetos_rbac_tab', activeTab); updateActiveTabs(); drawTab(); return; }
      const actionBtn = e.target.closest('[data-rbac-action]'); if (actionBtn) { handleAction(actionBtn.dataset.rbacAction); return; }
      const toastBtn = e.target.closest('[data-toast]'); if (toastBtn) notify(toastBtn.dataset.toast);
      const userBtn = e.target.closest('[data-open-user]'); if (userBtn) openUser(userBtn.dataset.openUser);
      const roleBtn = e.target.closest('[data-rbac-role]'); if (roleBtn) roleDrawer(Number(roleBtn.dataset.rbacRole));
      const policyBtn = e.target.closest('[data-rbac-policy]'); if (policyBtn) policyDrawer(Number(policyBtn.dataset.rbacPolicy));
      const inviteBtn = e.target.closest('[data-rbac-invite]'); if (inviteBtn) inviteReviewDrawer(Number(inviteBtn.dataset.rbacInvite));
      const closeBtn = e.target.closest('[data-rbac-close]'); if (closeBtn) document.getElementById('rbacDrawer')?.classList.remove('open');
    });
  }
  ...
  function selectedUser() { const id = localStorage.getItem('fleetos_selected_access_user') || users[1].id; return users.find(x => x.id === id) || users[1]; }
  function detailTab(name, active=false) { return `<button class="${active?'active':''}" type="button" data-detail-tab="${name}">${name}</button>`; }

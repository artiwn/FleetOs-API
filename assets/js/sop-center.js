(function () {
  const state = { tab: 'catalog', query: '', status: 'All Statuses', category: 'All Categories' };
  ...
  function escalationView() {
    return `<section class="panel glass-card workorder-panel sop-panel">
      ...
      <div class="incident-detail-callout sop-callout"><h3>Escalation principle</h3><p>FleetOS mirrors Bitrix stage changes but keeps SLA and escalation governance visible at platform level.</p></div>
    </section>`;
  }
  ...
  function createDrawer(kind = 'sop') {
    ...
    d.innerHTML = `<button type="button" class="drawer-close" data-sop-close>×</button>
      ...
      ${config.body}
      <div class="incident-detail-callout sop-callout sop-drawer-callout"><h3>Governance note</h3><p>This form only defines FleetOS routing and governance rules. Actual staff execution remains in Bitrix and syncs back through API.</p></div>
  ...
  function bind() {
    ...
    main.addEventListener('click', event => {
      const tab = event.target.closest('[data-sop-tab]');
      if (tab) { state.tab = tab.dataset.sopTab; render(); return; }
      const open = event.target.closest('[data-sop-open]');
      if (open) { location.href = `sop-detail.html?id=${encodeURIComponent(open.dataset.sopOpen)}`; return; }
      const action = event.target.closest('[data-sop-action]');
      if (action) {
        const name = action.dataset.sopAction;
        if (['create','mapping','evidence','escalation'].includes(name)) createDrawer(name);
        else FleetLayout.toast('SOP governance snapshot exported');
      }
    });
    main.addEventListener('input', event => {
      if (event.target.matches('[data-sop-search]')) { state.query = event.target.value; render(); }
    });
    main.addEventListener('change', event => {
      if (event.target.matches('[data-sop-status]')) { state.status = event.target.value; render(); }
      if (event.target.matches('[data-sop-category]')) { state.category = event.target.value; render(); }
    });
  }

  document.addEventListener('click', event => {
    const close = event.target.closest('[data-sop-close], [data-sop-close-toast]');
    if (!close) return;

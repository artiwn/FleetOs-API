const Commercial = (() => {
  ...
  const assignments = records.map(r => ({ client:r.client, tariff:r.plan, code:r.tariffCode, start:r.startDate, end:r.endDate, status:r.contractStatus, modules:r.modules }));
  let activeId = records[0].id;
  let activeGovernanceTab = 'models';

  const tenantCatalog = records.reduce((acc, item) => {
    acc[item.tenant] = acc[item.tenant] || [];
    acc[item.tenant].push({ client: item.client, plants: item.assignedPlants });
    return acc;
  }, {});
  ...
  const toast = msg => (window.FleetLayout && FleetLayout.toast) ? FleetLayout.toast(msg) : alert(msg);
  const byId = id => records.find(item => item.id === id) || records[0];
  ...
  function dataTable(heads, body, cls='commercial-billing-table-v120') {
  ...
  const rows = (items = records) => items.map(row).join('');
  ...
  function renderTariffAssignment(record) {
  ...
  function renderUsageCharges(record) {
    const body = [
  ...
  function renderInvoiceSnapshot(record) {
  ...
  function renderEnergySales(record) {
  ...
  function renderGovernanceModels() {
  ...
  function renderGovernanceContent(tab='models') {
  ...
  function registry() {
    return `<section class="page-hero commercial-hero-v78"><div><p class="eyebrow">Global Admin · Commercial Governance</p><h1>Commercial Models</h1><p class="muted">Tariff constructor, Rating Engine simulation, billing rules, discounts, assignments and invoice snapshot governance.</p></div><button class="primary-action" type="button" id="openCommercialCreate">Create Commercial Model</button></section>
  ...
  const stepOrder = () => ['scope','tariff','usage','rules','modules','payment','review'];
  function showCreateStep(step) { const order = stepOrder(); const idx = Math.max(0, order.indexOf(step)); document.querySelectorAll('[data-commercial-create-step]').forEach(btn => btn.classList.toggle('active', btn.dataset.commercialCreateStep === order[idx])); document.querySelectorAll('[data-commercial-create-panel]').forEach(panel => panel.classList.toggle('active', panel.dataset.commercialCreatePanel === order[idx])); const prev = document.querySelector('[data-commercial-create-prev]'); const next = document.querySelector('[data-commercial-create-next]'); const save = document.querySelector('[data-save-commercial-create]'); if (prev) prev.disabled = idx === 0; if (next) next.classList.toggle('hidden', idx === order.length - 1); if (save) save.classList.toggle('hidden', idx !== order.length - 1); }
  function moveCreateStep(delta) { const order = stepOrder(); const active = document.querySelector('[data-commercial-create-step].active')?.dataset.commercialCreateStep || order[0]; showCreateStep(order[Math.min(order.length - 1, Math.max(0, order.indexOf(active) + delta))]); }
  function closeModal() { document.getElementById('commercialCreateModal')?.remove(); }
  function updateCommercialCreateClientOptions() { const tenant = document.getElementById('commercialCreateTenant')?.value; const clients = tenantCatalog[tenant] || []; const clientSelect = document.getElementById('commercialCreateClient'); if (clientSelect) clientSelect.innerHTML = clients.map(x => `<option>${x.client}</option>`).join('') || '<option>No clients</option>'; updateCommercialCreatePlantOptions(); }
  function updateCommercialCreatePlantOptions() { const tenant = document.getElementById('commercialCreateTenant')?.value; const client = document.getElementById('commercialCreateClient')?.value; const selected = (tenantCatalog[tenant] || []).find(x => x.client === client); const plantSelect = document.getElementById('commercialCreatePlants'); if (plantSelect) plantSelect.innerHTML = `<option>${selected?.plants || 'No assigned plants'}</option>`; }
  ...
  function openDetail(id) { activeId = id || activeId; const root = document.getElementById('commercialRoot'); if (root) root.innerHTML = detail(byId(activeId)); window.scrollTo({ top: 0, behavior: 'smooth' }); }
  function showTab(tab) { document.querySelectorAll('[data-commercial-tab]').forEach(btn => btn.classList.toggle('active', btn.dataset.commercialTab === tab)); document.querySelectorAll('[data-commercial-panel]').forEach(panel => panel.classList.toggle('active', panel.dataset.commercialPanel === tab)); }


  function rerenderGovernance(tab = activeGovernanceTab) {
  ...
  function closeCommercialActionModal() { document.querySelector('[data-commercial-action-modal]')?.remove(); }

  function openCommercialActionModal(title, description, fields, onSave) {
  ...
  function wire() {
    document.addEventListener('click', e => {
      const proto = e.target.closest('[data-prototype-action]'); if (proto) { openPrototypeAction(proto.dataset.prototypeAction); return; }
      if (e.target.closest('[data-close-commercial-action]')) { closeCommercialActionModal(); return; }
      if (e.target.closest('#openCommercialCreate')) { createModal(); return; }
      if (e.target.closest('[data-close-commercial-create]')) { closeModal(); return; }
      if (e.target.closest('[data-save-commercial-create]')) { toast('Commercial model saved in prototype'); closeModal(); return; }
      const governanceTab = e.target.closest('[data-commercial-governance-tab]');
      if (governanceTab) { const tabName = governanceTab.dataset.commercialGovernanceTab; activeGovernanceTab = tabName; document.querySelectorAll('[data-commercial-governance-tab]').forEach(btn => btn.classList.toggle('active', btn === governanceTab)); rerenderGovernance(tabName); return; }
      const createStep = e.target.closest('[data-commercial-create-step]'); if (createStep) { showCreateStep(createStep.dataset.commercialCreateStep); return; }
      if (e.target.closest('[data-commercial-create-next]')) { moveCreateStep(1); return; }
      if (e.target.closest('[data-commercial-create-prev]')) { moveCreateStep(-1); return; }
      if (e.target.closest('[data-run-simulation]')) { renderSimulationResult(); toast('Rating simulation recalculated'); return; }
      const openBtn = e.target.closest('[data-action="open-commercial"]'); if (openBtn) { openDetail(openBtn.dataset.id); return; }
      const row = e.target.closest('.commercial-registry-row[data-commercial-id]'); if (row && !e.target.closest('.row-actions')) { openDetail(row.dataset.commercialId); return; }
      const tab = e.target.closest('[data-commercial-tab]'); if (tab) { showTab(tab.dataset.commercialTab); return; }
      if (e.target.closest('[data-back-commercial]')) { const root = document.getElementById('commercialRoot'); if (root) root.innerHTML = registry(); return; }
    });
    document.addEventListener('input', e => { if (e.target && e.target.id === 'commercialSearch') filterRows(); if (['simStations','simInverters','simCommands','simDiscount'].includes(e.target.id)) renderSimulationResult(); });
    document.addEventListener('change', e => { if (e.target && (e.target.id === 'commercialPlan' || e.target.id === 'commercialStatus')) filterRows(); if (e.target && e.target.id === 'commercialCreateTenant') updateCommercialCreateClientOptions(); if (e.target && e.target.id === 'commercialCreateClient') updateCommercialCreatePlantOptions(); if (e.target && e.target.id === 'simAI') renderSimulationResult(); });
  }

  function render() { FleetLayout.mount(`<div id="commercialRoot">${registry()}</div>`); wire(); }
  return { render };
})();

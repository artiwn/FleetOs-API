...
function alertCodeMeta(a){ return FleetAlertDictionary.codes[a?.fleetCode] || { category:a?.category || 'Unmapped', name:a?.title || 'Unknown alert', severity:a?.severity || 'Unknown', deviceScope:a?.deviceType || '—', policy:'No canonical policy configured yet.', meaning:a?.description || 'No unified explanation configured.', vendorMappings:[] }; }
...
function renderMappingValidation(a){
  ...
  const items = [
    ...
    { label:'FleetOS code assigned', hint: a.fleetCode || 'No canonical code', status: a.fleetCode ? 'Done' : 'Missing', checked: !!a.fleetCode, required:true },
...
const FleetAlerts = [
  {
    ...
    description: 'Vendor sent raw alert code 2010: EPM communication failure. FleetOS mapped it to FL-COM-RS.',
  ...
  },
  {
    ...
    description: 'Vendor sent raw alert code 2034: Grid Overvoltage. FleetOS mapped it to FL-GRD-OV.',
  ...
  },
  {
    ...
    description: 'Vendor sent raw alert code 3105: Battery Overtemperature. FleetOS mapped it to FL-BAT-OT.',
  ...
  },
  {
    ...
    description: 'Vendor sent raw status NET Red: Cloud disconnect. FleetOS mapped it to FL-COM-SRV.',
  ...
  },
  {
    ...
    description: 'Vendor sent raw alert code 31: Internal Comm Error. FleetOS mapped it to FL-INV-INT.',
  ...
  },
  {
    ...
    description: 'Vendor sent raw alert code 2011: Meter communication failure. FleetOS mapped it to FL-MTR-COM.',
...
function getAlertContext() {
  try { return JSON.parse(localStorage.getItem('fleetos_alert_context') || '{}'); } catch { return {}; }
}
function setAlertContextFromQuery() {
  const params = new URLSearchParams(location.search);
  const ctx = {};
  ['plantId','deviceId','tenant','status','severity'].forEach(k => { if (params.get(k)) ctx[k] = params.get(k); });
  if (Object.keys(ctx).length) localStorage.setItem('fleetos_alert_context', JSON.stringify(ctx));
}
function clearAlertContext(){ localStorage.removeItem('fleetos_alert_context'); }
...
function renderAlertFilters() {
  const ctx = getAlertContext();
  const opt = (value, selected) => `<option ${value === selected ? 'selected' : ''}>${value}</option>`;
  return `
    <section class="filter-bar glass-card alert-filter-bar">
      <label>Severity<select id="severityFilter">${['All','Critical','Fault','Warning'].map(x => opt(x, ctx.severity || 'All')).join('')}</select></label>
      <label>Status<select id="statusFilter">${['All','Open','Acknowledged','Escalated','Resolved'].map(x => opt(x, ctx.status || 'All')).join('')}</select></label>
      <label>Tenant<select id="tenantFilter">${['All','Tenant Alpha Energy','Tenant North Operations','Tenant Gamma Grid','Tenant Delta Enterprise'].map(x => opt(x, ctx.tenant || 'All')).join('')}</select></label>
      <label>Plant<select id="plantFilter">${['All', ...Array.from(new Set(FleetAlerts.map(a => a.plant)))].map(x => opt(x, 'All')).join('')}</select></label>
      <label>Vendor<select id="vendorFilter">${['All','Huawei','Sungrow','Solis','GoodWe','Deye'].map(x => opt(x, 'All')).join('')}</select></label>
      <label>Search<input id="alertSearch" placeholder="Alert, plant, device, category..." /></label>
    </section>`;
}
...
function alertRow(a) {
  ...
  return `
    <div class="data-row alert-row" data-alert-id="${a.id}">
      <div><strong>${a.fleetCode || '—'}</strong><small>Vendor: ${vendorCodeLabel(a)}</small></div>
...
function renderAlertsTable(items = filteredAlerts()) {
  return `
    <section class="panel glass-card">
      ...
      <div class="data-table alerts-table">
        <div class="data-head alert-head"><span>Alert Codes</span><span>Alert</span><span>Plant / Device</span><span>Source</span><span>Severity</span><span>Status</span><span>SLA</span><span>Actions</span></div>
        <div id="alertsRows">${items.length ? items.map(alertRow).join('') : '<div class="empty-state">No alerts match current filters.</div>'}</div>
      </div>
    </section>`;
}
...
function renderAlertDetailContent(a) {
  return `
    <section class="page-hero">
      <div><p class="eyebrow">Alert Detail</p><h1>${a.title}</h1><p class="muted">${a.id} · ${a.tenant} · ${a.plant} · ${a.device}</p></div>
...
function selectedAlert() {
  const params = new URLSearchParams(location.search);
  const id = params.get('id') || localStorage.getItem('fleetos_selected_alert') || FleetAlerts[0].id;
  return FleetAlerts.find(x => x.id === id) || FleetAlerts[0];
}

function openAlert(id) {
  localStorage.setItem('fleetos_selected_alert', id);
...
function applyAlertFilters() {
  ...
  if (kpiWrap) kpiWrap.innerHTML = alertKpis(items);
  if (rows) rows.innerHTML = items.length ? items.map(alertRow).join('') : '<div class="empty-state">No alerts match current filters.</div>';
}
...
function wireAlertsPage() {
  document.querySelector('.main-content').addEventListener('click', e => {
    ...
    if (e.target.closest('#clearAlertContext')) { clearAlertContext(); location.reload(); }
    if (e.target.closest('#exportAlerts')) FleetLayout.toast('Alert export queued');
  });
  ['severityFilter','statusFilter','tenantFilter','plantFilter','vendorFilter','alertSearch'].forEach(id => document.getElementById(id)?.addEventListener(id === 'alertSearch' ? 'input' : 'change', applyAlertFilters));
}
...
function wireAlertDetailPage() {
  ...
  document.querySelectorAll('.alert-detail-nav-v71 button').forEach(btn => btn.onclick = () => {
    ...
    document.getElementById('alertDetailContent').innerHTML = alertDetailTab(a, btn.dataset.tab);
...
function bindAlertDetailActions(a) {
  ...
  if (plant) plant.onclick = () => { localStorage.setItem('fleetos_selected_plant', a.plantId); location.href = 'plant-detail.html'; };
  const device = document.getElementById('openAlertDevice');
  if (device) device.onclick = () => { localStorage.setItem('fleetos_selected_device', a.deviceId); location.href = 'device-detail.html'; };
  const heroDevice = document.getElementById('openAlertDeviceFromHero');
  if (heroDevice) heroDevice.onclick = () => { localStorage.setItem('fleetos_selected_device', a.deviceId); location.href = 'device-detail.html'; };
  const tel = document.getElementById('openAlertTelemetry');
  if (tel) tel.onclick = () => { localStorage.setItem('fleetos_telemetry_context', JSON.stringify({ tenant: a.tenant, plant: a.plant, device: a.device, metric: a.related.telemetryMetric, range: localStorage.getItem('fleetos_time') || 'Last 24h', layer: 'Normalized' })); location.href = 'telemetry.html'; };
  ...
  const handlers = {
    ...
    actionTask: () => {
      ...
      localStorage.setItem('fleetos_task_context', JSON.stringify({ alertId: a.id, plant: a.plant, device: a.device, source: 'Alert Detail' }));
    },
    ...
    openAlertCase: () => {
      localStorage.setItem('fleetos_incident_context', JSON.stringify({ alertId: a.id, caseId: alertIncidentModel(a).caseId, plant: a.plant, device: a.device }));
  ...
  Object.entries(handlers).forEach(([id, fn]) => {
    document.querySelectorAll(`#${id}`).forEach(btn => btn.onclick = () => {
      const result = fn();
      if (result !== false) FleetLayout.toast(`${btn.textContent.trim()} updated for ${a.id}`);
      rerenderActiveAlertTab(a);
    });
  });
  ...
  document.querySelectorAll('.sop-check-input').forEach(input => {
    input.onchange = () => {
      ...
      done[Number(input.dataset.index)] = input.checked;
  ...
  if (outcome) outcome.onchange = () => {
    saveAlertRuntimeState(a, { outcome: outcome.value });
    rerenderActiveAlertTab(a);
  };
}


function alertRuntimeState(a) {
  try { return JSON.parse(localStorage.getItem(`fleetos_alert_runtime_${a.id}`) || '{}'); }
  catch { return {}; }
}
function saveAlertRuntimeState(a, patch) {
  const next = Object.assign({}, alertRuntimeState(a), patch || {});
  localStorage.setItem(`fleetos_alert_runtime_${a.id}`, JSON.stringify(next));
  return next;
}
function rerenderActiveAlertTab(a) {
  ...
  return `
    <section class="alert-detail-hero glass-card">
      ...
      <div class="alert-detail-meta-grid">
        ...
        <div><span>FleetOS Alert Code</span><strong>${a.fleetCode || '—'}</strong></div>
...
function renderAlertDetailContent(a) {
  ...
  return `
    <section class="page-hero alert-detail-page-hero">
      <div><p class="eyebrow">Global Admin · Alerts</p><h1>Alert Details</h1><p class="muted">Source-normalized alert workspace with incident, SOP, timeline and related objects.</p></div>
...
function alertDetailTab(a, tab) {
  const m = alertDetailModel(a);
  if (tab === 'classification') { const meta = alertCodeMeta(a); return `<div class="split-grid alert-classification-tab"><div class="panel-lite"><h3>FleetOS Unified Code</h3><div class="info-grid"><div><span>FleetOS Code</span><strong>${a.fleetCode || '—'}</strong></div><div><span>Unified Name</span><strong>${meta.name}</strong></div><div><span>Category</span><strong>${meta.category}</strong></div><div><span>Severity</span><strong>${meta.severity}</strong></div><div><span>Device Scope</span><strong>${meta.deviceScope}</strong></div><div><span>Meaning</span><strong>${meta.meaning}</strong></div></div></div><div class="panel-lite"><h3>Vendor Source Mapping</h3><div class="info-grid"><div><span>Vendor</span><strong>${a.vendor}</strong></div><div><span>Source Platform</span><strong>${a.source}</strong></div><div><span>Received Vendor Code</span><strong>${vendorCodeLabel(a)}</strong></div><div><span>Vendor Message</span><strong>${a.vendorMessage || a.title}</strong></div><div><span>Mapping Status</span><strong>${vendorMappingStatus(a)}</strong></div><div><span>Known Mapping</span><strong>${(meta.vendorMappings || []).join(' · ') || '—'}</strong></div><div><span>Integration</span><strong>${a.integration}</strong></div><div><span>Policy</span><strong>${meta.policy}</strong></div></div><div class="vertical-actions"><button onclick="location.href='alert-dictionary.html'">Open Alert Dictionary</button><button id="actionTask">Create Case from Policy</button></div></div><div class="panel-lite full-span-v86"><h3>Mapping Validation Checklist</h3>${renderMappingValidation(a)}</div></div>`; }

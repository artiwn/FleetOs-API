(function () {
  ...
  const current = sops[params.get('id')] || sops['SOP-001'];
  ...
  function sideNav() {
    const tabs = [['overview','Overview'], ['workflow','Workflow'], ['triggers','Trigger Rules'], ['routing','Routing Rules'], ['evidence','Evidence Rules'], ['versions','Version History'], ['audit','Audit']];
  ...
  function overviewView() {
    return `<section class="panel glass-card incident-detail-panel workorder-detail-panel sop-detail-panel">
      ...
      <div class="incident-detail-callout"><h3>Purpose</h3><p>This SOP decides what happens after a matching event is classified: whether FleetOS only notifies the client, creates a Bitrix ticket, starts a remote work order, or escalates to field service.</p></div>
    </section>`;
  }
  ...
  function triggersView() {
    return `<section class="panel glass-card incident-detail-panel workorder-detail-panel sop-detail-panel">
      <div class="panel-head"><div><h2>Trigger Rules</h2><p>Defines when FleetOS should apply this SOP automatically or offer it as a manual override.</p></div><button type="button" class="small-btn ghost" data-sop-detail-action="test-rule">Test Rule</button></div>
      ${infoGrid([
  ...
  function bind() {
    ...
    app.addEventListener('click', event => {
      const tab = event.target.closest('[data-sop-detail-tab]');
      if (tab) { state.tab = tab.dataset.sopDetailTab; render(); return; }
      const action = event.target.closest('[data-sop-detail-action]');

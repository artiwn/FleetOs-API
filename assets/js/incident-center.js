(() => {
  const state = { tab: 'incidents', filter: 'all', selected: null };
  ...
  const incidents = [
    {
      ...
      desc: 'Critical device alert routed to technical team because the device can potentially be recovered remotely from FleetOS or vendor API.'
    },
    {
      id: 'INC-3102',
      alertId: 'ALT-9024',
    ...
    },
    {
      ...
      source: 'FleetOS Connectivity Monitor',
  ...
  const syncEvents = [
    { time: '10:21', source: 'FleetOS', action: 'Created incident INC-3101 from normalized alert ALT-9017', result: 'Success' },
    { time: '10:22', source: 'Bitrix24', action: 'Task BX-TASK-8841 assigned to Technical Support L2', result: 'Success' },
    { time: '10:27', source: 'FleetOS', action: 'Remote diagnostics step started', result: 'In Progress' },
    { time: '10:31', source: 'Bitrix24', action: 'Status synchronized back to FleetOS', result: 'Success' }
  ];
  ...
  function kpi(label, value, note, filter, danger) {
  ...
  function routingRules() {
    return `<section class="panel glass-card incident-panel">
      <div class="panel-head"><div><h2>Routing Rules</h2><p>Classification decides whether FleetOS only notifies the client or creates a Bitrix ticket, task or work order.</p></div><button type="button" class="small-btn primary" data-create-incident-rule>Create Rule</button></div>
  ...
  function bitrixSync() {
    return `<section class="split-workspace incident-sync-layout">
      <div class="panel glass-card">
        <div class="panel-head"><div><h2>Bitrix Sync Queue</h2><p>Objects FleetOS receives back from Bitrix API: ticket status, assignee, stage, comments and work order evidence.</p></div><button type="button" class="small-btn primary" data-incident-toast="Manual Bitrix sync requested">Sync Now</button></div>
        <div class="data-table compact-table incident-sync-table">
          <div class="data-head"><span>Bitrix Object</span><span>FleetOS Incident</span><span>Owner</span><span>Status</span><span>Sync</span></div>
      ...
      <aside class="panel glass-card">
        <div class="panel-head"><div><h2>Latest Sync Events</h2><p>Traceability from FleetOS to Bitrix and back.</p></div></div>
  ...
  function analytics() {
    return `<section class="panel glass-card incident-panel">
      ...
      <div class="incident-flow-map">
        ...
        <div><span>04</span><strong>Synced back</strong><small>Stage, owner, SLA, comments and evidence visible in FleetOS.</small></div>
      </div>
    </section>`;
  }
  ...
  function ensureUtilityDrawer(id, title, subtitle, html) {
  ...
  function openRuleDrawer(key) {
    const r = routes.find(x => x.key === key) || routes[0];
    ensureUtilityDrawer('incidentUtilityDrawer', `${r.name} Routing Rule`, 'Controls how FleetOS classifies alerts and what is created in Bitrix.', `
      <div class="drawer-metrics rich incident-drawer-grid">
      ...
      <div class="incident-drawer-section"><h3>Result</h3><p>${r.key === 'info' ? 'Client notification is sent and the incident is closed as no action required.' : `A ${r.creates.toLowerCase()} is created in Bitrix and synced back to FleetOS.`}</p></div>
  ...
  function openBitrixDrawer(id) {
    ...
    ensureUtilityDrawer('incidentUtilityDrawer', `Bitrix Object ${i.bitrix}`, 'Read-only preview of the object synchronized from Bitrix API.', `
      ...
      <div class="incident-drawer-section"><h3>What FleetOS receives</h3><p>Assignee, stage, SLA state, comments, evidence links and resolution result.</p><small>${i.sync}</small></div>
  ...
  function openTimelineDrawer(id) {
    const i = incidents.find(x => x.id === id);
    if (!i) return;
    ensureUtilityDrawer('incidentUtilityDrawer', `${i.id} Timeline`, 'FleetOS + Bitrix execution history for this incident.', `
  ...
  function bind() {
    ...
    main.addEventListener('click', e => {
      const tab = e.target.closest('[data-incident-tab]');
      if (tab) {
        state.tab = tab.dataset.incidentTab;
        render();
        return;
      }
      const filter = e.target.closest('[data-incident-filter]');
      if (filter) {
      ...
      const open = e.target.closest('[data-open-incident], [data-incident-id]');
      if (open) {

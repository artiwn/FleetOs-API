(() => {
  ...
  const tone = window.FleetOSWorkOrders?.tone || ((value='') => /breach|overdue|failed|p1|critical/i.test(value) ? 'danger' : /risk|waiting|approval|scheduled|p2/i.test(value) ? 'warning' : 'success');
  const escape = window.FleetOSWorkOrders?.escape || (value => String(value ?? '').replace(/[&<>"]/g, c => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;' }[c])));

  const workOrders = window.FleetOSWorkOrders?.workOrders || [
    { id: 'WO-901', title: 'Remote inverter recovery sequence', type: 'Remote Action', priority: 'P1', stage: 'In Progress', owner: 'Technical Operations · Remote Team', assignee: 'Arman Petrosyan', tenant: 'Arpi Solar Group', client: 'Arpi Solar Group', plant: 'Berlin Solar 3', asset: 'INV-023 · Huawei SUN2000', sourceIncident: 'INC-3104', bitrixId: 'BX-TASK-8831', sla: '42 min remaining', sync: 'Synced 2 min ago', evidence: 'Pending command result', next: 'Run gateway restart and attach command log', desc: 'Technical team can try remote recovery from FleetOS before field visit is scheduled.' },
  ...
  ];  const order = workOrders.find(x => x.id === currentId) || workOrders[0];
  ...
  const timeline = [
    ...
    { time: '10:24', title: 'Bitrix object created', body: `${order.bitrixId} created and linked to FleetOS work order ${order.id}.` },
    { time: '10:31', title: 'Assigned to owner', body: `${order.owner} / ${order.assignee} accepted ownership.` },
    { time: '10:42', title: 'Execution step started', body: order.type === 'Field Visit' ? 'Field visit planning started in Bitrix and mirrored back to FleetOS.' : order.type === 'Support Case' ? 'Support contacted the client and is waiting for confirmation.' : 'Remote recovery checklist started by technical owner.' },
    { time: '10:52', title: 'Execution in progress', body: order.next },
    { time: '11:02', title: 'Sync back', body: `${order.sync}. FleetOS mirrored stage, SLA and evidence state.` },
  ...
  const audit = [
    ...
    { actor: 'FleetOS Sync', action: 'Pulled latest Bitrix task fields', time: 'Today · 11:02', result: 'Success' }
  ];
  ...
  function timelineView() {
    return `<section class="panel glass-card incident-detail-panel workorder-detail-panel">
      <div class="panel-head"><div><h2>Execution Timeline</h2><p>Mirrored from FleetOS routing, Bitrix stage updates and evidence events.</p></div></div>
  ...
  function bitrixView() {
    return `<section class="panel glass-card incident-detail-panel workorder-detail-panel">
      <div class="panel-head"><div><h2>Bitrix Mirror</h2><p>FleetOS does not replace Bitrix. It shows the operational state returned by Bitrix API.</p></div><button type="button" class="small-btn ghost" data-workorder-detail-action="sync">Sync</button></div>
      ${infoGrid([
        ['Bitrix Object', order.bitrixId], ['Current Stage', order.stage], ['Responsible', order.assignee], ['Queue', order.owner], ['Last Sync', order.sync], ['Mapped Fields', 'stage, owner, due date, comments, evidence state, linked incident'], ['External Link', `Open ${order.bitrixId} in Bitrix`], ['Sync Direction', 'Bitrix ↔ FleetOS']
      ])}
    </section>`;
  }
  ...
  function resolutionView() {
    ...
    return `<section class="panel glass-card incident-detail-panel workorder-detail-panel">
      ...
      <div class="incident-detail-callout"><h3>Closure Rule</h3><p>FleetOS should not mark the source incident as resolved until Bitrix stage, evidence state and resolution summary are aligned.</p></div>
    </section>`;
  }

  function relatedObjectsView() {
    return `<section class="panel glass-card incident-detail-panel workorder-detail-panel">
  ...
  function wire() {
    ...
    if (!main) return;
    main.onclick = event => {
      const tab = event.target.closest('[data-workorder-detail-tab]');
      if (tab) { state.tab = tab.dataset.workorderDetailTab; render(); return; }
      const action = event.target.closest('[data-workorder-detail-action]');
      if (!action) return;

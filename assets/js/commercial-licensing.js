const CommercialLicensing = (() => {
  ...
  let activePlanId = tariffPlans[0].id;
  ...
  function getActivePlan() {
    return tariffPlans.find(item => item.id === activePlanId) || tariffPlans[0];
  }

  function canPhysicallyDeletePlan(plan) {
    return String(plan?.status || '').toLowerCase() === 'draft' && Number(plan?.activeClients || 0) === 0;
  }

  function handlePlanRowKey(event, id) {
    if (event.key !== 'Enter' && event.key !== ' ') return;
    event.preventDefault();
    selectPlan(id);
  }

  function planRows(items = tariffPlans) {
  ...
  function deviceRows(planId = activePlanId) {
  ...
  function moduleCards(planId = activePlanId) {
  ...
  function selectPlan(id) {
    ...
    document.querySelectorAll('.tariff-plan-row').forEach(row => row.classList.toggle('selected', row.dataset.planId === id));
  ...
  function updateSimulatorDefaults(plan) {
    const fields = {
  ...
  function calculateInverters(qty) {
    const plan = getActivePlan();
  ...
  function toast(message) {
    if (window.FleetLayout?.toast) FleetLayout.toast(message);
  }
  ...
  function cloneTariff(planId = activePlanId) {
  ...
  function openDeleteTariff(planId = activePlanId) {
  ...
  function deleteTariff(planId = activePlanId) {
    ...
    if (canPhysicallyDeletePlan(plan)) {
      ...
      for (let i = devicePrices.length - 1; i >= 0; i -= 1) {
        if (devicePrices[i].tariffId === planId) devicePrices.splice(i, 1);
      }
      premiumModules.forEach(module => {
  ...
  function openAddModule() {
    ...
    const name = document.getElementById('newModuleName');
    const code = document.getElementById('newModuleCode');
    name?.addEventListener('input', () => { if (!code.dataset.touched) code.value = slugify(name.value); });
    code?.addEventListener('input', () => { code.dataset.touched = 'true'; });
  }
  ...
  function render() {
    ...
    return `
      <section class="page-hero commercial-hero tariff-hero">
        <div>
          <p class="eyebrow">Global Admin · Commercial Governance</p>

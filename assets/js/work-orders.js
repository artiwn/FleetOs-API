(() => {
  ...
  const workOrders = [
    {
      ...
      desc: 'Technical team can try remote recovery from FleetOS before field visit is scheduled.'
    },
    {
      id: 'WO-902',
  ...
  const boardStages = ['Scheduled', 'In Progress', 'Waiting Client', 'Done'];

  const syncRows = [
    { object: 'BX-TASK-8831', direction: 'Bitrix ↔ FleetOS', status: 'Healthy', last: '2 min ago', payload: 'Stage, assignee, SLA, comment thread' },
    { object: 'BX-TICKET-7762', direction: 'Bitrix → FleetOS', status: 'Healthy', last: '6 min ago', payload: 'Client communication and waiting status' },
    { object: 'BX-WO-4419', direction: 'FleetOS → Bitrix', status: 'Pending', last: 'Queued', payload: 'Field visit schedule update' },
    { object: 'BX-TASK-8840', direction: 'Bitrix ↔ FleetOS', status: 'Warning', last: '18 min ago', payload: 'Integration replay progress delayed' }
  ];
  ...
  const stats = [
    ...
    { label: 'Waiting Client', value: '9', note: 'Bitrix stage mirrored into FleetOS' },
  ...
  function boardView() {
    return `<section class="panel glass-card workorder-panel">
      <div class="panel-head"><div><h2>Execution Board</h2><p>High-level process view. Actual staff work stays in Bitrix; FleetOS mirrors stage and SLA.</p></div></div>
  ...
  function syncView() {
    return `<section class="panel glass-card workorder-panel">
      <div class="panel-head"><div><h2>Bitrix Work Order Sync</h2><p>Shows whether FleetOS has the latest stage, assignee, comments, due date and evidence state from Bitrix.</p></div><button type="button" class="small-btn ghost" data-workorder-action="sync-all">Sync All</button></div>
  ...
  function createDrawer() {
    drawer('Create Work Order', `<p class="eyebrow">Work Order Builder</p><h2>Create Work Order</h2>
      <div class="drawer-body">
        <p class="muted">This creates or mirrors a Bitrix task depending on routing source. In production it would call the Bitrix API and store the FleetOS work order reference.</p>
    ...
    FleetLayout.mount(`
      <section class="page-hero">
        <div>
          ...
          <p class="muted">Execution governance over Bitrix tasks, support cases, remote technical actions and field visits. FleetOS shows who is doing what, current stage, SLA, evidence and sync status.</p>
        </div>
  ...
  function wire() {
    ...
    if (!main) return;
    main.onclick = event => {
      const tab = event.target.closest('[data-workorder-tab]');
      if (tab) { state.tab = tab.dataset.workorderTab; render(); return; }

      const open = event.target.closest('[data-workorder-open]');
      if (open) {
      ...
      const action = event.target.closest('[data-workorder-action]');
      if (action) {
    ...
    main.querySelector('[data-workorder-search]')?.addEventListener('input', event => { state.query = event.target.value; render(); });
    main.querySelector('[data-workorder-status]')?.addEventListener('change', event => { state.status = event.target.value; render(); });
    main.querySelector('[data-workorder-type]')?.addEventListener('change', event => { state.type = event.target.value; render(); });
  }

  document.addEventListener('click', event => {
    const close = event.target.closest('[data-workorder-close], [data-workorder-close-toast]');
    if (!close) return;
  ...
  window.FleetOSWorkOrders = { workOrders, tone, escape };
  render();
})();

    ...
    const BillingConfiguration = (() => {
      const storageKey = 'fleetos.billing.configuration.v2';
      const seed = {
        activeTab: 'overview',
      ...
      function dataTable(heads, rows, cls = '') {
      ...
      function saveInvoice() {
        const subtotal = Number(document.getElementById('billSubtotal').value || 0);
        const markupPct = Number(document.getElementById('billMarkup').value || 0);
        const discountPct = Number(document.getElementById('billDiscount').value || 0);
        const taxPct = Number(document.getElementById('billTax').value || 0);
      ...
      function configureDunning() { FleetLayout.toast('Dunning policy configuration saved'); }
      function editDunning(stage) { FleetLayout.toast(`${stage} dunning stage opened`); }
      function recognizeRevenue() { FleetLayout.toast('Revenue recognition batch prepared'); }
      ...
      function syncErp(system) {
        FleetLayout.toast(`${system} sync started`);
      }
      ...
      return { mount, closeModal, openInvoiceModal, saveInvoice, openChargeModal, saveCharge, openCycleModal, saveCycle, openTaxModal, saveTaxRule, openSimulatorModal, runSimulation, openCreditModal, saveCreditAdjustment, addDeposit, configureDunning, editDunning, recognizeRevenue, selectInvoice, advanceInvoice, openSnapshot, runBillingCycle, addUsageRecord, testRating, syncErp, exportBatch };
    })();

    BillingConfiguration.mount();
